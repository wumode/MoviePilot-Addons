...
class ClashRuleProviderService:
    ...
    @staticmethod
    def _country_from_node(countries: list[dict[str, str]], node_name: str) -> dict[str, str] | None:
    ...
    def get_ruleset(self, name: str) -> str | None:
    ...
    async def async_get_subscription(
            self, url: str,
            ua: str | None = None
    ) -> tuple[ClashConfig | None, SubscriptionInfo | None]:
        if not url:
            return None, None
        logger.info(f"正在刷新 {UtilsProvider.get_url_domain(url)} ...")
        ret = None
        raw_proxies = {}
        ua = ua if ua else "clash.meta"
        headers = {
            "User-Agent": ua,
        ...
        for _ in range(self.state.config.retry_times):
            ret = await AsyncRequestUtils(
                headers=headers, timeout=self.state.config.timeout,
