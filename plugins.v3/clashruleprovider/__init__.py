...
from app.sdk.config import settings, global_vars
...
class ClashRuleProvider(_PluginBase):
    ...
    def init_plugin(self, config: dict | None = None):
        ...
        if config:
            try:
                self.state.config = PluginConfig.model_validate(config)
    ...
    async def refresh_subscription_service(self):
        ...
        if self.state.config.notify:
            self.post_message(
                title=f"【{self.plugin_name}】", mtype=MessageType.Plugin, text='\n'.join(messages)
            )
    ...
    def update_cloudflare_ips_handler(self, event: Event):
        ...
        if not event_data or not isinstance(event_data, dict) or event_data.get("action") != "update_cloudflare_ips":
            return
